...
exports.createOrder = async (req, res) => {
  try {
    ...
    // Prepare order data for Cashfree PRODUCTION
    const orderData = {
      order_id: orderId,
      order_amount: parseFloat(amount).toFixed(2), // Must be "XX.XX" string
      order_currency: "INR",
      customer_details: {
      ...
      order_meta: {
        // Cashfree requires EXACTLY this format for return_url in production
        return_url: `${process.env.FRONTEND_URL}/payment-success?order_id=${orderId}&status={order_status}`,
    ...
    const response = await axios.post(
      ...
      orderData,
      {
        headers: {
          ...
          "x-api-version": "2023-08-01", // Updated to latest stable
    ...
    console.log("✅ Order created successfully:", {
      orderId,
      paymentSessionId:
        response.data.payment_session_id.substring(0, 20) + "...",
    });
    ...
    if (error.response) {
      ...
      errorMessage =
        "No response from Cashfree API. Check your internet connection.";
    }
...
exports.testConnection = async (req, res) => {
  try {
    ...
    const response = await axios.get(`${process.env.CASHFREE_API_URL}/orders`, {
      headers: {
        "x-client-id": process.env.CASHFREE_APP_ID,
        "x-client-secret": process.env.CASHFREE_SECRET_KEY,
        "x-api-version": "2023-08-01",
  ...
  try {
    ...
    if (event === "ORDER.PAYMENT.CAPTURED") {
      ...
      if (transaction && transaction.status === "PENDING") {
        ...
        console.log(
          `✅ Webhook: Balance updated for user ${transaction.userId}, added ₹${transaction.amount}`
        );
      }
    }
